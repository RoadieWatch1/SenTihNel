-- ============================================================
-- Add device ownership guard to register_or_move_device.
--
-- BUG: If User A's device_id somehow ends up in User B's
-- handshake (stale AsyncStorage, shared physical device),
-- the RPC silently reassigns user_id — User A loses their device.
--
-- FIX: Before upserting, check if the device_id already belongs
-- to a DIFFERENT user who was active in the last 24 hours.
-- If so, reject the handshake instead of stealing the device.
-- ============================================================
...
BEGIN
  ...
  -- NEW: Ownership guard — prevent cross-user device takeover.
  -- If the device_id belongs to a DIFFERENT user who was active recently, reject.
  IF EXISTS (
    ...
    RETURN jsonb_build_object('ok', false, 'error', 'Device belongs to another active user');
  END IF;

  -- Capture the old group_id BEFORE upserting (for cross-fleet cleanup)
  ...
  -- If device moved to a DIFFERENT fleet, mark old fleet's tracking_session as OFFLINE
